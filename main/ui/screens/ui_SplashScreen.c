...
ui_tomatotentText = lv_img_create(ui_SplashScreen);
lv_img_set_src(ui_tomatotentText, &ui_img_tomatotent_text_png);
lv_obj_set_width( ui_tomatotentText, LV_SIZE_CONTENT);  /// 240
lv_obj_set_height( ui_tomatotentText, LV_SIZE_CONTENT);   /// 50
lv_obj_set_x( ui_tomatotentText, 2 );
lv_obj_set_y( ui_tomatotentText, -67 );
lv_obj_set_align( ui_tomatotentText, LV_ALIGN_CENTER );
lv_obj_add_flag( ui_tomatotentText, LV_OBJ_FLAG_HIDDEN | LV_OBJ_FLAG_ADV_HITTEST );   /// Flags
lv_obj_clear_flag( ui_tomatotentText, LV_OBJ_FLAG_SCROLLABLE );    /// Flags
